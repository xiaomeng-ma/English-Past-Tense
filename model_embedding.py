...
value_name = args.embedding + '.data'
sound_name = args.embedding + '.sound'

values = np.loadtxt(value_name)
with open(sound_name, 'r') as sound_file:
  sounds = sound_file.read().strip().split(' ')
embedding_index = {}
for i in range(len(sounds)):
  embedding_index[sound[i]] = values[i]

num_tokens = len(values) + 3
