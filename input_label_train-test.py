...
def test_step(test_dataset, model):
    start, end = 1,2
    for element in test_dataset.as_numpy_iterator():
        ...
        for line_tar, line_output, lab in zip(tar.tolist(), output_list[0].numpy().tolist(), label.tolist()):
            ...
            output_a = "".join(t.sequences_to_texts([line_output[1:index_output]])[0].split(' '))
            tar_a = "".join(t.sequences_to_texts([line_tar[1:index_tar]])[0].split(' '))
            test_cor.append([output_a, tar_a, lab])
        df_test_cor = pd.DataFrame(test_cor, columns = ['pred','ipa_word', 'reg'])
...
def nonce_step(nonce_dataset, model):
    start, end = 1,2
    for (batch, (inp,tar)) in enumerate(nonce_dataset):
        ...
        for line_tar, line_output in zip(tar.numpy().tolist(), output_list[0].numpy().tolist()):
            ...
            output_a = "".join(t.sequences_to_texts([line_output[1:index_output]])[0].split(' '))
            tar_a = "".join(t.sequences_to_texts([line_tar[1:index_tar]])[0].split(' '))
            nonce_cor.append([output_a, tar_a])
        df_nonce_pred = pd.DataFrame(nonce_cor, columns = ['pred','ipa_word'])
...
if __name__ == "__main__":
    ...
    train_dataset, val_dataset, val_dataset_eval, test_dataset, nonce_dataset = dataset
    ...
    if ckpt_manager.latest_checkpoint:
        ...
        for epoch in range(EPOCHS):
            ...
            dev_reg, dev_irreg, dev_reg_char, dev_irreg_char, k = dev_step(val_dataset)
            val_k.append(k)
            logging.info(f'Epoch {epoch + 1} Dev Reg {dev_reg:.4f} Dev Irreg {dev_irreg:.4f} Dev Reg Char {dev_reg_char:.4f} Dev Irreg Char {dev_irreg_char:.4f}')
            print(f'Epoch {epoch + 1} Dev Reg {dev_reg:.4f} Dev Irreg {dev_irreg:.4f} Dev Reg Char {dev_reg_char:.4f} Dev Irreg Char {dev_irreg_char:.4f}')
            dev_acc = 0.5*(dev_reg) + 0.5*(dev_irreg)
    ...
    dev_reg_acc, dev_irr_acc , df_dev_pred, _ = test_step(val_dataset_eval, gen_model.model)
    test_reg_acc, test_irr_acc, df_test_pred, test_entorpy_list = test_step(test_dataset, gen_model.model)
    df_nonce_pred, nonce_entropy_list = nonce_step(nonce_dataset, gen_model.model)
    ...
    pred_list = gen_model.forward(test_dataset)
    df_top = pd.DataFrame(pred_list.numpy().tolist()).transpose()
    for key in df_top.columns:
        df_top[key] = df_top[key].apply(lambda row: t.sequences_to_texts([row])[0].split(' ')[1:])
        try:
            df_top[key] = df_top[key].apply(lambda row: "".join(row[:row.index('end')]))
        except:
            df_top[key] = df_top[key].apply(lambda row:"".join(row))
